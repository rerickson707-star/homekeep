// supabase/functions/drip-day10/index.ts
// Day 10: Warranties + documents
// Deploy: npx supabase functions deploy drip-day10 --no-verify-jwt
...
serve(async () => {
  ...
  // Get warranty count per user for personalization
  ...
  for (const w of warrantyCounts || []) {
    warrantyMap[w.user_id] = (warrantyMap[w.user_id] || 0) + 1;
  }
  ...
  for (const user of targets) {
    ...
    const wCount = warrantyMap[user.id] || 0;

    const warrantyLine = wCount > 0
      ? `You already have <strong>${wCount} warranty${wCount > 1 ? "s" : ""}</strong> tracked. `
      : `You haven't added any warranties yet. `;
...
<body style="margin:0;padding:0;background:#ECE3D2;font-family:'Helvetica Neue',Arial,sans-serif;">
  <div style="max-width:580px;margin:40px auto;background:#FBF7EE;border-radius:16px;overflow:hidden;">
    ...
    <div style="padding:40px;">
      <p style="font-size:13px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#C16140;margin:0 0 12px;">Protect what you own</p>
      <h1 style="font-family:Georgia,serif;font-size:24px;color:#234A3D;font-weight:400;margin:0 0 16px;line-height:1.3;">
        Hi ${name} — warranties expire whether you track them or not
      </h1>
      <p style="font-size:15px;color:#5A534B;line-height:1.7;margin:0 0 20px;">
        ${warrantyLine}Most homeowners discover a warranty has expired only after something breaks. Steadwell alerts you at 30 days and 7 days before anything expires — so you have time to file a claim or extend coverage.
      </p>

      <div style="background:#F4EDDF;border-radius:12px;padding:20px 24px;margin:0 0 20px;">
        <p style="font-size:13px;font-weight:700;color:#234A3D;margin:0 0 10px;">What to add to your warranty tracker:</p>
        <ul style="margin:0;padding:0 0 0 18px;font-size:14px;color:#5A534B;line-height:1.9;">
          <li>Refrigerator, washer, dryer, dishwasher</li>
          <li>HVAC system and water heater</li>
          <li>TV, computer, and electronics</li>
          <li>Roof, windows, and major systems</li>
          <li>Any recent home improvement with a contractor guarantee</li>
        </ul>
      </div>

      <p style="font-size:14px;color:#8A8178;margin:0 0 28px;">Also — your <strong>Document Vault</strong> is a secure place to store your deed, permits, inspection report, insurance policy, and any other important home document. Searchable, always accessible.</p>

      <div style="text-align:center;">
        <a href="${APP_URL}" style="background:#C16140;color:#fff;text-decoration:none;padding:14px 32px;border-radius:40px;font-size:14px;font-weight:700;display:inline-block;">Add your warranties →</a>
      </div>
    </div>
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from: FROM, to: [user.email!], subject: `${name}, when does your fridge warranty expire?`, html }),
    });
